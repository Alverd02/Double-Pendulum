...
while running:
    ...
    if mouse_state[0] == True:
        ...
        coor2 = (old_x2*100+new_center[0],old_y2*100+new_center[1])
        index = 0
        l1 = np.sqrt((coor1[0]/100-center[0]/100)**2 + (coor1[1]/100-center[1]/100)**2)
        l2 = np.sqrt((coor2[0]/100-coor1[0]/100)**2 + (coor2[1]/100-coor1[1]/100)**2)
        if coor1[0]<center[0]:
            theta_01 = -np.arccos(coor1[1]/(l1*100))
        else:
            theta_01 = np.arccos(coor1[1]/(l1*100))
        if coor2[0]<coor1[0]:
            theta_02 = -np.arccos(coor1[1]/(l1*100))
        else:
            theta_02 = np.arccos(coor1[1]/(l1*100))
        x1 = motion(theta_01,theta_02, l1, l2, omega0)[0]
        y1 = motion(theta_01,theta_02, l1, l2, omega0)[1]
